resource "local_file" "playbook_k8s" {
  ...
  content     = templatefile("./modules/dev/k8s/ansible/playbook_k8s.tpl",
  {
    external_nat_ip = yandex_compute_instance.nat.network_interface.0.nat_ip_address
})
  filename = "./modules/dev/k8s/ansible/playbook_k8s.yml"
}
...
resource "null_resource" "predeploy-k8s" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i ${yandex_compute_instance.nat.network_interface.0.nat_ip_address}, ./modules/dev/k8s/ansible/playbook_k8s.yml --key-file ./ssh/id_rsa"
  }
}
